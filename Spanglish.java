...
private static final int FRAMES_PER_BUFFER = 400; // 25ms of audio (0.025s* 16000Hz) FIX #2: The URL string was broken across two lines in the original code.
...
System.out.println("Audio will be saved to a WAV file when the session
ends.");
try {
...
System.out.println("Speak into your microphone. Press Ctrl+C to
stop.");
...
byte[] buffer = new byte[FRAMES_PER_BUFFER * 2]; // 2 bytes per sample (16-bit) -> FIX #5: Comment was broken in two lines here 
...
System.err.println("Error streaming audio: " +
e.getMessage());
}
break;
}
...
double durationSeconds = (double) totalDataSize / (SAMPLE_RATE *
CHANNELS * 2);
...
private void writeWavHeader(OutputStream out, int dataSize) throws
IOException {
...
public AssemblyAIWebSocketClient(URI serverUri, Map<String, String>
headers) {
...
String formattedTime = DateTimeFormatter.ofPattern("yyyy-MM-dd
HH:mm:ss")
.withZone(ZoneId.systemDefault())
.format(instant);
System.out.printf("%nSession began: ID=%s, ExpiresAt=%s%n",
sessionId, formattedTime);
}
...
System.out.printf("%nSession Terminated: Audio Duration=%.2fs,
Session Duration=%.2fs%n",
audioDuration, sessionDuration);
}
@Override
public void onClose(int code, String reason, boolean remote) {
System.out.printf("%nWebSocket Disconnected: Status=%d, Msg=%s%n",
code, reason);
stopRequested.set(true);
}
@Override
